# 🌦️ Weather App (React + TypeScript + Prisma + PostgreSQL)

Веб-приложение погоды с **React (Vite)** - фронтенд и **Node.js + TypeScript + Prisma** - бэкенд.

Проект находится **в одном репозитории без разделения на client/server**, сервер и фронтенд запускаются из корня.

---

## 🚀 Стек технологий

**Frontend**

* React
* Vite
* JavaScript / TypeScript
* CSS

**Backend**

* Node.js
* TypeScript
* Prisma ORM
* PostgreSQL

---

## 📁 Структура проекта

```text
weather2/
├── prisma/                # Prisma schema и миграции
├── src/                   # React frontend
│   ├── assets/
│   ├── generated/
│   ├── App.jsx
│   ├── main.jsx
│   └── index.css
├── public/
├── server.ts              # Backend сервер (Node.js + TS)
├── prisma.config.ts
├── vite.config.js
├── index.html
├── .env
├── package.json
├── pnpm-lock.yaml
└── README.md
```

---

## ⚙️ Предварительные требования

Установите:

* **Node.js** (v18+)
* **pnpm / npm**
* **PostgreSQL**

PostgreSQL должен быть запущен локально:

```
localhost:5432
```

---

## 🗄️ Настройка базы данных

1. Создайте базу данных в PostgreSQL с помощью Pgadmin (например `weather_db`)

2. В корне проекта укажите путь к базе данных в файле **.env**:

```env
DATABASE_URL="postgresql://USER:PASSWORD@localhost:5432/weather_db"
```

3. Примените миграции Prisma:

```bash
npx prisma migrate dev
```

## ⛅ Вставка АПИ

Возьмите АПИ Openweathermap и вставьте его на место key

```App.jsx
const api = {
  key: 'xxxxxxxxxxxxxxxxxxxxxxx',
  base: 'https://api.openweathermap.org/data/2.5/'
}
```

---

## 📦 Установка зависимостей

В корне проекта:

```bash
pnpm install
# или
npm install
```

---

## ▶️ Запуск backend-сервера

Сервер запускается из корня проекта:

```bash
npx nodemon server.js
```

По умолчанию:

```
http://localhost:3000
```

---

## ▶️ Запуск frontend-приложения

В новой вкладке терминала:

```bash
npm run dev
# или
pnpm dev
```

Frontend будет доступен по адресу:

```
http://localhost:5173
```
---

## 🛠️ Полезные команды

```bash
npx prisma studio      # GUI для базы данных
npx prisma generate   # Генерация Prisma Client
```

